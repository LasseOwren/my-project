    ...
    return items.map((item) => (
      <div key={item.id} className="product-card bg-white shadow-md rounded-lg p-6 mb-6 flex items-center">
        ...
        <div>
          ...
          <button className="bg-yellow-500 text-white py-2 px-6 rounded-full hover:bg-yellow-600">Bestill</button>
        </div>
      </div>
    ));
  };
  ...
  return (
    <div className="menu">
      ...
      <div className="products p-4">
        {category === 'Mat' ? renderSubcategories(products.Mat) : renderProducts(products[category])}
      </div>
